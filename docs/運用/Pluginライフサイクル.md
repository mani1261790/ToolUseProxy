...
- upgrade先: 現在の検証済みrelease candidate、Plugin `0.1.0-alpha.4`、SQLite schema v6
...
testはloopback HTTPだけを使う一時Git marketplaceを作り、同じrefをalpha.1から現在commitへfast-forwardします。`codex plugin marketplace upgrade tooluseproxy --json`後に、Plugin versionがalpha.4へ変わること、古いcacheが除かれること、`PLUGIN_DATA/events.db`が残ること、明示`init --codex`でmigration backupを作ってactiveになることを確認します。外部networkや実secretは使いません。
...
3. alpha.4をinstallし、旧schemaに対するHookがfail-openしてDDLやevent書込みをしないことを確認
...
6. alpha.4 codeをremoveしてdataを保持し、同じversionを再installしてruntime設定のrevisionと有効値が残ることを確認
...
9. alpha.1 runtimeがactiveになり、baseline eventを保持し、backup後のalpha.4 eventとruntime設定を含まないことを確認
