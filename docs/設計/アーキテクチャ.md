...
6. Stopはfinal answerの再確認を求め、opt-inのBash/MCP PreToolUseはcriticalなexternal sinkを実行前にdenyする
...
- Hook内のnetwork access、remote embedding、telemetryは使わない
- 未初期化または互換性のないDBをHookが暗黙migrationせず、diagnosticをstderrへ出してfail-openする
- `PostToolUse`は記録を行い、`Stop`はfinal answerを検査する
- Bash PreToolUse blockは既定で無効、MCP blockは別のopt-inも必要とする
