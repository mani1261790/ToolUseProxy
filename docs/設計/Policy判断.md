...
operation単位fragment、snapshot capture、複数workspaceのsource/cursor/resource分離、MCP exact profileと全scalar value / JSON key sink coverage、redaction preview planner、hash-only audit、future rendered planだけを対象にしたdormant Post confirmationは実装済みです。`PermissionRequest`は公式source、実payload、deny / allow E2Eを検証し、汎用runtime接続を追加しないと判断しました。PreToolUseはcurrent callの全critical findingを確定した後にpreview plannerを呼び、findingが参照するworkspace-owned source chunk IDだけを32件以下で取得します。1件でも非対応なら候補全体をrejectし、bounded envelope内のeligible / rejected planと全targetをimmutableな1 transactionで保存します。新規保存は完了runのcritical lineageとpure planner再実行結果へ完全一致させます。Post confirmationは保存済みpayload、最小Post sequence、verified preview clone、stable profileを再証明しますが、現行preview planは遷移させません。source取得、planner、保存、confirmationの失敗時も先にrenderしたdenyまたはPost eventを維持し、`updatedInput`は返しません。
