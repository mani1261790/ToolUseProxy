...
現在の実装は、Hook I/Oの文字列類似度、Filesystem adapterによるread/writeのstructured edge、Search/Bash/MCP/final answer adapterによるsink candidate、Stop hookの実継続、Bash/MCPの実行前遮断、session差分解析を扱うMVPです。

Stop / PreToolUse runtimeでは`analysis_cursors`にsessionごとの処理済みsequenceを保存し、canonical fragmentの5-gramを`fragment_shingles`へ索引化します。通常は新fragmentと同一sessionの候補だけを比較し、新edgeからlineage scoreが改善する範囲だけを伝播します。detectorまたはsource manifestが変わった場合は、そのsessionだけをfull rebuildします。
...
4. PermissionRequestと安全なredactの必要条件を評価する
