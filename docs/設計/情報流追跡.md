...
現在の実装は、Hook I/Oの文字列類似度、Filesystem adapterによるread/writeのstructured edge、Search/Bash/MCP adapterによるsink candidateを扱うMVPです。
...
2. lineageをCLIまたはGraphvizで可視化する
3. 正常・危険シナリオでedge精度を評価する
4. embeddingとコード向け特徴量を追加して比較する
5. PolicyDecisionをCodex Hook出力へ接続する
