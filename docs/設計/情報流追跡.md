...
現在の実装は、Hook I/Oの文字列類似度、Filesystem adapterによるread/writeのstructured edge、Search/Bash/MCP/final answer adapterによるsink candidate、Stop hookの実継続とsession差分解析を扱うMVPです。
...
1. Search / MCPの実payloadを確認する
2. MCP external sinkをPreToolUseへ接続する
3. resource versionのsnapshot captureを設計する
