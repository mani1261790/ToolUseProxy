...
Stop runtimeでは`analysis_cursors`にsessionごとの処理済みsequenceを保存し、canonical fragmentの5-gramを`fragment_shingles`へ索引化します。通常のStopでは新fragmentと同一sessionの候補だけを比較し、新edgeからlineage scoreが改善する範囲だけを伝播します。detectorまたはsource manifestが変わった場合は、そのsessionだけをfull rebuildします。
...
1. Bash external sinkをPreToolUseの`deny`へ接続する
2. Search / MCPの実payloadを確認する
3. MCP external sinkをPreToolUseへ接続する
4. resource versionのsnapshot captureを設計する
