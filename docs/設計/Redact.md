...
2026-07-16には同じ最大caseで、各sinkのversion照合がimmutable profile / registryのsemantic JSONとSHA-256を繰り返し生成していたhot pathを除去しました。profile / registry versionをfrozen object上でcacheし、planner内でもversionをhoistします。またoriginal / rewrittenのstructure hashを意味とrejection順序を変えず4回から2回へ減らしました。200 warmup後に3 caseを各2,000回interleaveした再測定は、単一target p95 0.0735 ms、最大eligible p95 0.8107 ms、最大reject p95 0.5497 msでした。eligibleの単発max 17.604 msとrejectの9.928 msはp95と分けて扱います。cache後もprofile / registryのpickle・deepcopy互換、semantic changeによるversion変化、全finding / sink metadata検証を回帰testで維持します。
