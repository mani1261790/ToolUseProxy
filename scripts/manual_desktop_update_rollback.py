...
def _old_baseline_prompt(init_command: str) -> str:
    return (
        ...
        "承認画面ではMarkdownを使わず、次の1段落を表示してください。\n"
        "ToolUseProxyの操作確認｜行うこと：旧版の検証用DBとworkspace登録を初期化"
        "します｜変更されるもの：指定した検証用Plugin dataとworkspace登録｜外部"
        "通信：ありません｜確認が必要な理由：プロジェクト外の専用保存領域へ書き"
        "込み、update前のbaselineを作るためです｜"
        "この内容で実行してよいですか？\n\n"
        f"{init_command}\n\n"
...
def _write_update_context_and_prompt(
    ...
    prompt = (
        ...
        "ください。形式は「ToolUseProxyの操作確認｜行うこと：...｜変更されるもの："
        "...｜外部通信：ありません｜確認が必要な理由：...｜この内容で実行してよい"
        "ですか？」です。行うことと変更範囲を具体的に書き、ToolUseProxy側から許可"
        "や拒否を指示しないでください。どれかが"
