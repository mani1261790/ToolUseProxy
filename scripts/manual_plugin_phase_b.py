...
def prepare_phase_b(root_argument: Path) -> dict[str, Any]:
    ...
    _write_private(
        ...
        (
            ...
            "｜通信：なし｜理由：承認が必要な理由｜許可：許可してよい範囲\n\n"
            "この説明では `#`、`*`、backtick、箇条書き、表を使いません。"
            "過去のガイドを覚えていなくても理解できる必要があります。"
            "許可には、今回のsubcommandと対象範囲、contextとの照合結果、"
            "拒否すべき不一致を短く書きます。長いsh自体を理解しないと"
    ...
    prompt = (
        ...
        "改行がすべて消えても全角ラベルで意味を区切れるようにし、全体を短くしてください。"
        "過去のガイドを覚えていなくてもその場だけで判断できる自己完結した説明にしてください。"
        "許可では今回のsubcommandと対象範囲をもう一度書き、exact command argumentsから"
        ...
        "私が「守る」を選んだ場合だけapproveし、再scanとstatusを確認してください。"
