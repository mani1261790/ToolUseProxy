...
このbranchはv0.2.0-alpha.2の開発版です。公開チャンネルや現在のインストールが更新されたとは限りません。
...
2. ToolCallのI/O・登録メタデータ・必要な資源の証拠が判定用Codexへ送られ、バックグラウンド解析もモデルを利用することを確認します。判定不能時は警告して通すので、その間の流出防止を保証しません。
