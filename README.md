...
> このbranchは **v0.2.0-alpha.1の開発版**です。公開済みv0.1の導入手順とは異なります。

## 主な機能

- **守る情報を選ぶ** — 利用者が指定したファイルを、確認してから登録します。初期化だけで、すべてのファイルが保護対象になることはありません。
- **送信前に確認する** — ToolCallの記録を独立したCodex判定モデルに渡し、内容の依存関係と外部通信の可能性を調べます。
- **危険な操作を止める** — 対応するツール操作について、外部へ送る内容から登録済み情報源へ依存経路が見つかった場合に実行を止め、理由を示します。判定不能や時間切れは警告して継続します。
...
設定では、ToolCallの入力・出力を判定用Codexへ渡すことと、判定不能時に警告して継続する方針を説明します。
...
ToolCallをノードとし、LLMが内容の依存エッジを推定します。外部通信の際に登録情報源への到達性を機械的に調べます。保護情報の見逃しを減らすことと、公開してよい情報の送信を不必要に止めないことが研究課題です。
