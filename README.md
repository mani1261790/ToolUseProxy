...
> 現在は`0.1.0-alpha.3` public alphaです。中核機能、再現可能な配布物、Apache-2.0の配布契約、CLI TUIでのfile-backed exact-only enforcement検証は整いましたが、完全なDLPではありません。Codex DesktopではPluginの検索・install、Hook review、trusted Pre / Post / Stop probeまで確認しました。通常の`workspace-write` taskではworkspace外のPlugin dataへ初期化する1コマンド単位の権限昇格が必要であり、更新版skillで再検証中です。adapter外のnetwork egress、hosted Web Search、Linux実Codex task、Windows実機も引き続き検証中です。
...
| Public alpha | `0.1.0-alpha.3` | Apache-2.0、checksum / SBOM、archive内部、CI Action、hash-locked build、Git履歴監査、upgrade / rollback、自動dogfoodを完了。CLI TUIのfile-backed shadow / exact-only Phase Bは合格。Desktopはinstall、trust、実Hook probeまで合格し、workspace外Plugin dataへの1コマンド単位の権限昇格とprotected blockを再検証中。cross-platform実機、少人数pilotは継続課題 |
...
Hook trustで確認する内容、更新チャンネルと固定tagの違い、protected sourceの候補発見・承認、rollback、削除時のdata保持を含む完全な手順は[Plugin導入](docs/設定/Plugin導入.md)にあります。Codex Desktopはlocal Pluginを利用できるsurfaceで、[Desktop専用Phase B harness](docs/運用/DesktopPhaseB.md)まで実装済みです。2026-07-30の最新runでは、定義hashを固定した3 Hookのtrustと、値を含まないmarkerによるPre / Post / Stop各1回の実配送を確認しました。一方、`workspace-write` taskからworkspace外のPlugin dataへ初期化するにはexact commandの権限昇格が必要で、通常権限の`init`はOS拒否となりました。更新版skillで1コマンド単位の承認を要求し、public allow / protected blockまで再確認するまでは、Desktop / GUI上の保護を利用可能とは扱いません。Codex CLIのMarketplace更新と保護動作は実機検証済みです。
