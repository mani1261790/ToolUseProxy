...
複数workspaceの分離も実装済みです。明示したcanonical workspace rootをidentityとし、event、source、cursor、resource、edge、解析runをworkspaceごとに分離します。runtimeは現在eventのworkspaceとsessionだけを更新し、offline CLIは`--analysis-run ID`または`--workspace-root PATH --latest`の明示を必須にします。completed offline runはedgeだけでなくnode metadataもcontent-addressed snapshotとして保持するため、後からlive DBのnodeが変わっても過去runを再現できます。`PermissionRequest`は実payload、公式実装、実Codexのdeny / allowを検証した結果、PreToolUseの代替にならず、現時点では汎用runtime接続を追加しないと判断しました。redactは現行Codexの複数rewrite競合を考慮してruntimeへ即接続せず、versioned MCP exact profile、全argument scalar value / JSON keyのsink coverage、call内全critical findingをまとめるpure preview plannerまで実装しました。external write-like MCP inputはartifact生成前にbounded preflightし、plannerもsourceとtargetを直接参照して全件対応できる場合だけwhole-field候補を作ります。Hook stdoutは従来のdeny / 空stdoutのままで、次はpreview plan / targetのhash-only監査保存です。
