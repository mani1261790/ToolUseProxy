...
本プロジェクトは[SecHack365](https://sechack365.nict.go.jp/)での研究・開発成果物です。この版は`0.1.0-alpha.16`です。質問文と選択肢だけを表示する既知の操作と、固定の`git remote`一覧表示で起きる不要な停止を減らしました。保護情報を含む質問、未知の関数、動的な命令、外部送信の停止は維持します。alpha.15の7秒の内部制限と大きなDB向けの索引も維持しています。研究用の固定実通信試験は通常Pluginへ同梱せず、自動起動もしません。研究用public alphaであり、完成したDLP製品ではありません。
...
> **以前の版から更新する場合:** `alpha.16`へ更新し、Codexを完全に終了して起動し直した後、新しいタスクで動作を確認してください。Hook定義が変更された場合は5 Hookを改めて確認します。`alpha.12`以前には、Pluginを利用しないprojectにも初期化案内が出る旧問題もあります。
...
| Plugin配布 | alpha.16 | 未設定projectの無影響確認、判定時間切れ時の実行前停止、current-invocation照合、lifecycle、artifact、full testのrelease gateを通過したcommitだけを`public-alpha`へ配信 |
