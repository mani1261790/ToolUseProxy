...
This version is `0.1.0-alpha.17`. It retains the literal-question and fixed `git remote` false-block fixes, and fixes alpha.16's missed match when a protected string is split across question fields. Both individual values and ordered concatenated display views are compared. Unknown functions, dynamic commands, and external submissions remain conservative. Alpha.15's seven-second watchdog and session-scoped indexes remain in place. It is a research alpha, not a complete DLP system.
...
After upgrading to alpha.17, fully restart Codex and verify behavior in a new task. Review all five Hooks again if their definitions changed. Alpha.12 and earlier could show initialization diagnostics in projects where ToolUseProxy had never been enabled.
