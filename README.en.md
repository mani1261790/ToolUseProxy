...
This version is `0.1.0-alpha.18`. It adds a revision-bound operation that removes one protected-source registration while preserving the source file, other registrations, settings, and a private backup. The exact installed Plugin command is recognized as a local self-operation; appended shell commands, other workspaces, and malformed commitments are not. Earlier false-block, split-question matching, and watchdog fixes remain in place. It is a research alpha, not a complete DLP system.
...
After upgrading to alpha.18, fully restart Codex and verify behavior in a new task. Review all five Hooks again if their definitions changed. Alpha.12 and earlier could show initialization diagnostics in projects where ToolUseProxy had never been enabled.
