...
function select(n) {
  ...
  analysis.append(el('strong',n.complete?(['selected_output','selected_resources','selected_values'].includes(n.provenance_scope)?'使用部分の来歴を解析済み':'来歴解析済み'):'依存関係に未解析あり'),el('p',n.complete?(['selected_output','selected_resources','selected_values'].includes(n.provenance_scope)?'後続の呼び出しが使う出力やファイルの由来を解析しました。操作全体の解析結果とは異なります。':'この呼び出しの依存関係を解析しました。'):'一部の依存関係は未解析です。検査済み安全を意味しません。'));
