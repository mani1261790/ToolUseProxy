...
def make_workspace_id(canonical_root: str) -> str:
    digest = hashlib.sha256(canonical_root.encode("utf-8", errors="surrogateescape")).hexdigest()
...
def make_configured_workspace_namespace(configured_root: str) -> str:
    ...
    return f"{WORKSPACE_CONFIGURED_NAMESPACE_VERSION}_{hashlib.sha256(identity).hexdigest()}"
