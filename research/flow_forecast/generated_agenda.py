"""Reserve one generation call and seal its agenda plan before execution."""
import argparse
...
from .agenda_plan_provider import TASK, Plan, AgendaPlanProvider, definition, prompt
...
def prepare(provider, output, *, timeout=60, clock=time.monotonic):
    task = TASK
...
def main(argv=None):
    ...
    if args.stage == 'prepare':
        ...
        result = prepare(AgendaPlanProvider(args.model), args.output)
    else:
        if args.prepared is None or args.repository is None:
            parser.error('collect requires --prepared and --repository')
        from .agenda_collection import run
        value = load(args.prepared)
        result = run(args.repository, value['plan']['export'], args.output, generation=value)
