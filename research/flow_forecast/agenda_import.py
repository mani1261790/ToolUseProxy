"""Read closed agenda captures without claiming independent tasks or generators."""
    ...
    try:
        ...
        for condition in conditions:
            ...
            for number, row in enumerate(rows, 1):
                ...
                script = dispatch_script(row['call'], number, intent['variant'], row['receiver_address'], row['step_id'])
                ...
                expected = {'call_sha': row['call_sha'], 'script_sha': row['script_sha'], 'oracle_matched': True,
                            'dispatch': {'call': row['call'], 'before': expected_state(number > 1),
                                         'after': expected_state(True),
                                         'output': expected_output(number, intent['variant']) if number < 3 else {'delivered': True}},
                            'observer': {'state': expected_state(True),
                                         'value': expected_output(min(number, 2), intent['variant'])}}
                if number == 3:
        ...
        raise ForecastDataError('invalid_agenda_evidence') from error


def dataset(intent, execution, report):
    """API state/selection relations stay unknown; receiver bytes alone are known."""
    validate(intent, execution, report)
    parsed = []
...
def read_capture(directory):
    if directory.is_symlink() or not directory.is_dir():
        raise ForecastDataError('invalid_agenda_directory')
    total = 0
    def read(name):
        nonlocal total
        ...
        total += len(raw)
        if total > 32 * 1024**2:
            raise ForecastDataError('agenda_import_size_limit')
        return _json(raw)
    try:
        ...
        for name in ('hook_monitor/evaluation/flow_lab/agenda_api.py', 'research/flow_forecast/agenda_transport.py'):
        ...
        if isinstance(error, ForecastDataError):
            raise
        raise ForecastDataError('invalid_agenda_capture') from error


def catalog():
    raw = b'# Closed synthetic agenda family\n\nOwner-scoped creation, public/private query and isolated delivery. Related development variants; independence and prior nonuse unverified.\n'
    sha = hashlib.sha256(raw).hexdigest()
    return {'schema': 1, 'designs': [{'id': 'agenda', 'objective': 'Create and query an owner-scoped agenda record and deliver its returned JSON.',
        'origin': {'kind': 'new_design', 'artifact_sha': sha, 'rationale': 'Closed synthetic stateful API; one related development family.'},
        'parents': [], 'tools': ['tool_output', 'http'],
        'flow': [['input', 'agenda-add', 'record'], ['record', 'agenda-query', 'response'], ['response', 'send', 'receiver']],
        'success': 'Exact declared API outputs, state and receiver bytes match.',
