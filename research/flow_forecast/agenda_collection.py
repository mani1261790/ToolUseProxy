"""One finite synthetic agenda batch: JSON dispatch, guard and isolated receiver."""
import argparse
...
from .agenda_transport import AgendaTransport, dispatch_script
...
def run(repository, variant, output, *, seconds=180, clock=time.monotonic):
    ...
    def charge(identity):
        ...
        check()
        if charged >= 20:
            raise LabError('agenda_trial_budget_exhausted')
        charged += 1
    ...
    try:
        ...
        with TrialStore(output / 'controls') as store:
            for mode in ('observe', 'enforce'):
                check()
                spec = RunSpec(uuid.uuid4().hex, 'agenda-dispatch-v1', 'source-' + execution['context_sha'],
                               'fixed-exact-externality-v1', image[7:], utc_now(), max_trials=20)
                store.start(spec)
                transport = AgendaTransport(image, variant, mode)
                ...
                with transport:
                    ...
                    if not (all(c.observer_state == 'complete' for c in controls)
                        ...
                        raise LabError('agenda_controls_failed')
                    ...
                    for number in range(1, 4):
                        ...
                        if not dispatched and transport.delivery(step_id) != ('no', 'no'):
                            raise LabError('agenda_denied_dispatch_mismatch')
                    ...
                    if not transport.inspect(transport.receiver, network=transport.network).get('State', {}).get('Running'):
                        raise LabError('agenda_receiver_unavailable')
...
def main(argv=None):
    ...
    try:
        report = run(args.repository, args.variant, args.output, seconds=args.seconds)
