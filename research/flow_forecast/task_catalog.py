...
def collect_message_captures(directories, source_path, *, checked=False):
    ...
    for directory in directories:
        if checked:
            from .checked_message import read_checked_capture
            if (type(directory) is not dict or set(directory) != {'capture','interventions'}
                    or any(type(v) is not str or not v for v in directory.values())):
                raise ForecastDataError('invalid_checked_message_input')
            data, audit = read_checked_capture(Path(directory['capture']), Path(directory['interventions']), source)
        else:
            data, audit = read_capture(directory, source)
        total += len(canonical(audit).encode()) + len(canonical(asdict(data)).encode())
...
def main(argv=None):
    ...
    inputs_group.add_argument('--checked-message-captures', type=Path, help='JSON list of Message capture/interventions pairs')
    ...
    if args.message_source and not (args.message_captures or args.checked_message_captures):
        parser.error('--message-source requires --message-captures')
    if args.checked_message_captures:
        if args.catalog or args.origins or args.message_source is None:
            parser.error('checked Message captures require --message-source and use their captured definitions')
        pairs = _json(_read(args.checked_message_captures))
        if type(pairs) is not list:
            raise ForecastDataError('invalid_checked_message_input')
        data, audit, catalog, origins = collect_message_captures(tuple(pairs), args.message_source, checked=True)
    elif args.message_captures:
        if args.catalog or args.origins or args.message_source is None:
