...
from research.flow_forecast import generated_agenda as module, agenda_collection, agenda_import, task_catalog, generator_strata
from research.flow_forecast.agenda_plan_provider import AgendaPlanProvider, Plan, prompt, TASK
from test_flow_lab_codex_agent import events, executable
from test_flow_forecast_agenda_import import capture

PLAN = {'status':'propose','operations':['add','query','send'],'export':'public'}


def provider(tmp_path, value=PLAN):
    return AgendaPlanProvider('fixture-model', executable=executable(tmp_path, f'print({events(value).decode()!r})'))
...
@pytest.mark.parametrize('value', [{**PLAN,'operations':['add','send']},
...
def test_capture_binds_generation_before_trials_and_rejects_variant_mismatch(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(agenda_collection,'build_context',fail)
    with pytest.raises(LabError,match='fixture_preflight'):
        agenda_collection.run(tmp_path,'public',tmp_path/'capture',generation=frozen)
    with pytest.raises(LabError,match='plan_mismatch'):
        agenda_collection.run(tmp_path,'include_private',tmp_path/'wrong',generation=frozen)
...
def test_original_receipt_reaches_collection_and_cannot_be_reused(tmp_path):
    ...
    for i in range(2):
        ...
        _,audit = agenda_import.read_capture(path)
    ...
    for count in (1,2):
        ...
        task_catalog.main(['--agenda-captures',str(inputs),'--output',str(directory)])
