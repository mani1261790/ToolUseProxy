...
def lab(monkeypatch):
    ...
    monkeypatch.setattr(module, 'execute', lambda image, source: subprocess.check_output(
...
def test_failure_keeps_charge_and_no_completion(lab, monkeypatch, tmp_path):
    monkeypatch.setattr(module, 'execute', lambda *a: b'wrong')
    output = tmp_path / 'batch'
