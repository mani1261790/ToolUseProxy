...
class ManualDesktopPhaseBTest(unittest.TestCase):
    ...
    def test_desktop_prompt_requires_per_command_sandbox_escalation(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            prompt = (root / "desktop-phase-b-prompt.txt").read_text(encoding="utf-8")
    ...
    def test_hooks_list_requires_exact_trusted_phase_b_hooks(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            hook_root = (
                codex_home / "plugins" / "cache" / "tooluseproxy" / "tooluseproxy" / "0.1.0-alpha.3"
            )
            workspace.mkdir()
            ...
            self.assertTrue(all(item["trust_status"] == "trusted" for item in inventory["hooks"]))
    ...
    def test_hooks_list_allows_generated_python_cache(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            (python_cache / "runtime.cpython-312.pyc").write_bytes(b"generated bytecode")
    ...
    def test_instrumented_launcher_records_real_hook_dispatch(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            instrumented = json.loads((hooks_root / "hooks.json").read_text(encoding="utf-8"))
    ...
    def test_checkpoint_hook_probe_removes_probe_gate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            hooks = [
                ...
                for index, event in enumerate(("PostToolUse", "PreToolUse", "Stop"))
            ]
            ...
            state.update(
                {
                    ...
                    "probe_nonce": probe_nonce,
                    "trusted_hook_hashes": {item["event"]: item["current_hash"] for item in hooks},
                }
            )
            ...
            with (
                ...
                patch(
                    "scripts.manual_desktop_phase_b._read_desktop_probe_session",
                    return_value={
    ...
    def test_abort_plan_recovers_exact_plugin_installed_before_checkpoint(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            current["marketplaces"] = [{"name": MARKETPLACE_NAME, "root": str(marketplace)}]
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            def run_json(
                ...
                **_: object,
                ...
                if arguments[0:2] == [
                    "sh",
                    str(Path(str(state["marketplace"])) / PLUGIN_NAME / "hooks" / "run_cli.sh"),
                ]:
    ...
    def test_cleanup_apply_refuses_launcher_change_before_child_process(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with (
                ...
                planned = plan_cleanup(root)
            launcher = Path(str(state["marketplace"])) / PLUGIN_NAME / "hooks" / "run_cli.sh"
            ...
            with (
                ...
                apply_cleanup(
                    root,
                    confirmation_token=planned["local_only"]["confirmation_token"],
                )
    ...
    def test_cleanup_retry_after_data_deletion_does_not_delete_twice(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual("restored", result["status"])
            self.assertFalse(any("uninstall" in call and "apply" in call for call in calls))
    ...
    def test_cleanup_partial_deletion_requires_fresh_review_and_resumes(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertFalse(any("marketplace" in call for call in first_calls))
            ...
            self.assertEqual("restored", result["status"])
            apply_calls = [call for call in second_calls if "uninstall" in call and "apply" in call]
            self.assertEqual(1, len(apply_calls))
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with (
                ...
                first_review = apply_cleanup(
                    root,
                    confirmation_token=initial["local_only"]["confirmation_token"],
                )
            with (
                patch(
                    "scripts.manual_desktop_phase_b._capture_shared_state",
            ...
            with self.assertRaises(DesktopPhaseBFailure) as raised:
                apply_cleanup(
                    root,
                    confirmation_token=first_review["local_only"]["confirmation_token"],
                )
            self.assertEqual(
                "confirmation_token_invalid",
                raised.exception.code,
    ...
    def test_prepare_returns_desktop_search_without_unsupported_deep_link(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with (
                ...
                patch(
                    "scripts.manual_desktop_phase_b._run_json",
                    return_value={"marketplaceName": ("tooluseproxy-desktop-phase-b")},
                ),
            ):
    ...
    def test_session_parser_measures_scoped_plugin_data_calls(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            command = (
                f"sh {launcher} status --workspace {workspace} --data-dir {plugin_data} --json"
            )
            records = [
                {
                    "type": "session_meta",
    ...
    def test_session_parser_flags_unscoped_or_reusable_plugin_data_call(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            launcher = plugin_root / "hooks" / "run_cli.sh"
            command = (
                f"sh {launcher} doctor --workspace {workspace} --data-dir {plugin_data} --json"
            )
            records = [
                {
                    "type": "session_meta",
    ...
    def test_plugin_data_comes_from_exact_trace_path_without_search(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            selected = _plugin_data_from_session(
                (),
                (f"Trace: tooluseproxy trace --db {plugin_data / 'events.db'} --analysis-run run",),
                codex_home=codex_home,
                plugin_root=plugin_root,
            )
    ...
    def test_release_zip_extracts_marketplace_and_plugin_as_siblings(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertTrue(
                (destination / "tooluseproxy" / ".codex-plugin" / "plugin.json").is_file()
            )
    ...
    def _uninstall_plan(
        ...
        *,
        ...
        return {
            "status": ("review_required" if review_required else "nothing_to_delete"),
            "data_dir": str(Path(str(state["plugin_data"])).resolve()),
