...
*** End Patch"""
    ...
    def test_pre_tool_runner_allows_exact_removal_when_source_is_missing(
        self,
    ) -> None:
        ...
        command = (
            f"sh {launcher} protect remove plan --path private.py "
            f"--workspace {workspace} --data-dir {self.db_path.parent} --json"
        )

        exit_code, stdout, stderr = self._run_hook_in_process(
            "pre_tool_use",
            {
                "session_id": "session-missing-source-remove",
                "turn_id": "turn-missing-source-remove",
                "tool_use_id": "bash-missing-source-remove",
                "tool_name": "Bash",
                "cwd": str(workspace),
                "tool_input": {"command": command},
            },
