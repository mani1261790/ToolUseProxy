...
powershell -NoProfile -NonInteractive -Command "$p=(Get-Location).ProviderPath; $d=$env:PLUGIN_DATA; $a=if($d){Join-Path $d 'events.db.workspaces'}else{$null}; $marked=$a-and(Test-Path -LiteralPath $a -PathType Container)-and(@(Get-ChildItem -LiteralPath $a -Filter 'ws_v1_*.json' -File).Count-gt0); $complete=$marked-and(Test-Path -LiteralPath (Join-Path $a 'migration-v1.complete') -PathType Leaf); while($true){if($marked){$s=[Security.Cryptography.SHA256]::Create(); $h=([BitConverter]::ToString($s.ComputeHash([Text.Encoding]::UTF8.GetBytes($p)))).Replace('-','').ToLower(); $s.Dispose(); if(Test-Path -LiteralPath (Join-Path $a ('ws_v1_'+$h+'.json')) -PathType Leaf){exit 0}}; if((-not $complete)-and(Test-Path -LiteralPath (Join-Path $p 'protected_sources.json') -PathType Leaf)){exit 0}; if(Test-Path -LiteralPath (Join-Path $p '.git')){if($marked-and(-not $complete)){exit 0}; exit 1}; $q=Split-Path -Parent $p; if([string]::IsNullOrEmpty($q)-or$q-eq$p){if($marked-and(-not $complete)){exit 0}; exit 1}; $p=$q}"
exit /b %errorlevel%
