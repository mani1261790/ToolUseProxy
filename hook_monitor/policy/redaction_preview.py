...
PLANNER_VERSION = "mcp-redaction-preview-v1"
REPLACEMENT_TEXT = "[REDACTED BY TOOLUSEPROXY]"
REPLACEMENT_PROFILE = "whole_field_fixed_v1"


@dataclass(frozen=True)
class RedactionPreviewLimits:
    input_limits: McpInputLimits = DEFAULT_MCP_INPUT_LIMITS
    max_critical_findings: int = 32
    max_distinct_targets: int = 16
    max_source_bytes_per_finding: int = 32 * 1024
    max_source_bytes_total: int = 128 * 1024
...
_REJECTION_RANK = {
    rejection_code: index for index, rejection_code in enumerate(_REJECTION_ORDER)
}
...
def plan_mcp_redaction_preview(
    *,
    ...
    source_chunks: Mapping[tuple[str, str], SourceChunk],
...
def _structure_sha256(value: object) -> str | None:
    try:
...
def _top_level_pointer_value(arguments: dict[str, Any], pointer: str) -> object:
    try:
        return arguments[_top_level_pointer_key(pointer)]
    except (KeyError, ValueError):
        return None


def _replace_top_level_pointer(
    arguments: dict[str, Any],
    pointer: str,
    replacement: str,
) -> None:
    key = _top_level_pointer_key(pointer)
