...
判定時は、同じプロジェクトの関連するToolCall記録と登録済み情報源のメタデータを、独立した`codex exec`へ渡します。来歴はセッションをまたぐ場合があります。送信内容を確かめるため、Hookは対象ファイルや必要なローカル実行定義を上限付きで読み取ります。取得した証拠が判定入力に含まれる場合があります。保護ファイルも内容一致の索引更新に読み取りますが、登録操作そのものでは本文を読みません。
