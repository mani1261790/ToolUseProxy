...
ToolUseProxy `0.1.0-alpha.24`は研究用public alphaです。未設定projectでは案内・記録・停止を行わず、明示設定済みprojectだけでHookを動かします。固定の公開Issue読み取り、既知の質問表示、固定のGit接続先一覧の不要な停止を減らし、判定が7秒以内に完了しない場合もCodex側の制限時間より先に安全な停止を返します。自動整理は新しいHook操作を検知すると全体調査を中断し、厳密一致するローカル管理操作はDB不調時にも復旧経路として許可します。外部補助プログラムを起動できる`core.fsmonitor`を明示的に無効化した`git status`だけをローカル操作として扱い、通常の`git status`や見た目だけ似た管理命令、通信を付け足した命令には例外を適用しません。本番環境向けのSLA、security certification、完全なDLP、全toolの遮断保証は提供しません。
